...
# Created by write_sdc on Tue Sep 30 21:09:58 2025
...
create_generated_clock [get_pins clk_div2_inst/div_clk]  -name UART_RX_CLK  -source [get_ports UART_CLK]  -master_clock UART_CLK  -divide_by 1  -add
set_clock_uncertainty -setup 0.2  [get_clocks UART_RX_CLK]
set_clock_uncertainty -hold 0.1  [get_clocks UART_RX_CLK]
create_generated_clock [get_pins clk_div_inst/div_clk]  -name UART_TX_CLK  -source [get_ports UART_CLK]  -master_clock UART_CLK  -divide_by 32  -add
set_clock_uncertainty -setup 0.2  [get_clocks UART_TX_CLK]
set_clock_uncertainty -hold 0.1  [get_clocks UART_TX_CLK]
create_clock [get_ports scan_clk]  -name CLK_scan  -period 100  -waveform {0 50}
set_clock_latency 0  [get_clocks CLK_scan]
set_clock_uncertainty -setup 0.2  [get_clocks CLK_scan]
set_clock_uncertainty -hold 0.1  [get_clocks CLK_scan]
set_clock_transition -min -fall 0.1 [get_clocks CLK_scan]
set_clock_transition -max -fall 0.1 [get_clocks CLK_scan]
set_clock_transition -min -rise 0.1 [get_clocks CLK_scan]
set_clock_transition -max -rise 0.1 [get_clocks CLK_scan]
set_input_delay -clock UART_CLK  54.2534  [get_ports UART_RX_IN]
set_input_delay -clock CLK_scan  20  [get_ports {SI[2]}]
set_input_delay -clock CLK_scan  20  [get_ports {SI[1]}]
set_input_delay -clock CLK_scan  20  [get_ports {SI[0]}]
set_input_delay -clock CLK_scan  20  [get_ports SE]
set_input_delay -clock CLK_scan  20  [get_ports test_mode]
...
set_output_delay -clock CLK_scan  20  [get_ports {SO[2]}]
set_output_delay -clock CLK_scan  20  [get_ports {SO[1]}]
set_output_delay -clock CLK_scan  20  [get_ports {SO[0]}]
set_clock_groups -asynchronous -name REF_CLK_1 -group [list [get_clocks REF_CLK] [get_clocks ALU_Gated_CLK]] -group [list [get_clocks UART_CLK] [get_clocks UART_TX_CLK] [get_clocks UART_RX_CLK]] -group [get_clocks CLK_scan]
